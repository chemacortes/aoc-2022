...
def read_input():
            ...
            if node != a_node and node.name in a_node.neighbor_map:
                ...
                for my_neighbor in node.neighbor_map:
                    ...
                    if my_neighbor not in a_node.neighbor_map:
                        a_node.neighbor_map[my_neighbor] = (
                            current_distance + node.neighbor_map[my_neighbor]
                        )
                    else:
    ...
    return {
        node.name: node
        for node in nodes
        if node.name == starting_node or node.flow
    }
    ...
    child_scores = max(
        [
            traverse(
                ...
                neighbor,
                new_visited,
                time + node.neighbor_map[neighbor] + 1,
                max_time,
            )
            for neighbor in node.neighbor_map
...
def part2():
    ...
    elephant_score, path2 = traverse(node_map, starting_node, max_time=26)
